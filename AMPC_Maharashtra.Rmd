...
Below are plots containing deseasonalized prices (multiplicative 
decomposition), with the raw and minimum support prices as reference. 

```{R, time_series_decomposition, echo=FALSE, results='hide', fig.keep='all'}
# Data preparation
...
`RicyPaddyHus` prices in Nagpur have been relatively stable compared to the 
other commodities studied. Deseasonalized prices are in general lower than the 
raw prices of the commodity. `Bajri` prices in Jalgaon and `Wheat Husk` prices 
in Dhule went through large fluctuations in 2015 & 2016 respectively. 

The behavior of the prices of the commodities as grouped by the location can be 
explored independently using similar graphs. The obtained inferences can be
then be examined through factors unforseen through the data.

## Flagging APMC/Commodity Pairs with Max Fluctuations
```{R, flagging_AMPCC}
...
print_table(head(fluctuation_data[order(-fluctuation_data$fluctuation),], 10))
```
...
## Conclusion
1. The outliers have been removed. 
2. The data is non-stationary; detected seasonality is multiplicative. 
3. The price fluctuations have been examined using selcted AMPC-Commodity pairs.
4. The most significant fluctuations have been flagged.